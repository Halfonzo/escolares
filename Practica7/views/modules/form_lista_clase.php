...
<table id="sample-data-table" class="table">
	<thead>
        ...
        $listado = $db->especial("SELECT id_materia,(SELECT nombre FROM materias WHERE id = id_materia) FROM grupoclase WHERE id_grupo = " . $_GET['value']);
		?>
        <tbody>
		<?php
		...
		?>
			<tr>
                ...
                <td>
		        <a onclick="if(confirm('Se eliminara al Alumno')){document.location.href='index.php?action=eliminarGrup-Clase&id=<?php echo $t_id ?>&clase='+document.getElementById('grupo').value}"><i class="icon s-7 icon-account-remove"></i></a>
                </td>
            </tr>	
		<?php
			}
